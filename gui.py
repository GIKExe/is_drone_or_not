...
def create_spectrogram(audio_array):
    """
    Раньше тут была своя копия логики (n_mels=128, ручная адаптивная
    нормализация по min/max буфера + повторный plt.imsave) — она не
    совпадала ни с тем, на чём обучалась модель (main.py: n_mels=512,
    обрезка FREQ_LOW:FREQ_HIGH, фиксированный VMIN_DB/VMAX_DB), ни с
    тем, что теперь делает test.py.

    Теперь используется ТА ЖЕ функция single_chunk_to_image, что и в
    test.py — GUI и консольный test.py видят статистически одинаковые
    картинки, совпадающие с обучающими.
    """
...
class DroneDetectorGUI:
    ...
    def setup_ui(self):
        ...
        self.indicator_text = self.indicator_canvas.create_text(100, 100, text="ОЖИДАНИЕ", fill='#888888',
                                                                font=('Arial', 14, 'bold'))
    ...
    def detection_loop(self):
        ...
        duration = 0.5
        frames_to_read = int(duration * SAMPLE_RATE)  # SAMPLE_RATE из spectrogram.py,
        # а не хардкод — гарантированно
        # совпадает с тем, что видела модель
