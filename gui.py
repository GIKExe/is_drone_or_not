...
class DroneDetectorApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="Акустическая система наблюдения",
                 font=("Segoe UI", 10),
        ...
        if self.models:
            ...
            tk.Label(model_panel, text="No .pt models found",
        ...
        stats_frame = tk.Frame(self.root, bg=BG_MAIN, pady=15)
        ...
        for i, (key, title) in enumerate([
            ...
            card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0 if i == 0 else 5, 0))
            card_inner = tk.Frame(card, bg=BG_CARD, padx=15, pady=12)
            card_inner.pack(fill=tk.BOTH, expand=True)
            tk.Label(card_inner, text=title,
                     font=("Segoe UI", 9, "bold"),
                     fg=TEXT_SECONDARY, bg=BG_CARD).pack(anchor=tk.W)
            value_label = tk.Label(card_inner, text="0.00%",
                                   font=("Segoe UI", 20, "bold"),
                                   fg=TEXT_PRIMARY, bg=BG_CARD)
            value_label.pack(anchor=tk.W, pady=(5, 0))
            self.cards[key] = value_label

        footer = tk.Frame(self.root, bg=BG_MAIN, pady=10)
        footer.pack(side=tk.BOTTOM, fill=tk.X)
        tk.Label(footer, text="v1.0  •  YOLO Acoustic Classifier",
                 font=("Segoe UI", 9),
                 fg=TEXT_SECONDARY, bg=BG_MAIN).pack()
    ...
    def update_gui(self):
        try:
            while True:
                ...
                if q == "error":
                    self.status_label.config(text=f"ERROR: {aq}", fg=ACCENT_RED)
                    self.stop_detection()
                    break
                self.cards["current"].config(text=f"{q*100:.2f}%")
                self.cards["avg"].config(text=f"{aq*100:.2f}%")
                self.cards["min"].config(text=f"{mq*100:.2f}%")
                self.set_indicator_state(is_drone)
                if is_drone:
                    self.status_label.config(text="ДРОН ОБНАРУЖЕН", fg=ACCENT_RED)
                else:
                    self.status_label.config(text="ПРОВЕРКА", fg=ACCENT_GREEN)
        except queue.Empty:
            pass
